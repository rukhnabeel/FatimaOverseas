...
export default function NewGroupFarePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/group-fares", {
        method: "POST",
      ...
      setError(err instanceof Error ? err.message : "Failed to save group fare");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-brand-dark">Post New Group Fare</h1>
          <p className="text-sky-500 mt-1">Create a new discounted flight for groups.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-sky-100 space-y-6">
        <div className="grid md:grid-cols-2 gap-6">
          <div className="space-y-2 md:col-span-2">
            ...
            <input required type="text" name="title" value={form.title} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="e.g. Indigo Delhi to Jeddah" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Airline</label>
            <input required type="text" name="airline" value={form.airline} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="e.g. Indigo Airlines" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Sector/Route</label>
            <input required type="text" name="sector" value={form.sector} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="e.g. DEL-JED-DEL" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Display Price Label (e.g. ₹55,000)</label>
            <input required type="text" name="priceLabel" value={form.priceLabel} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="e.g. ₹55,000" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Numeric Price (in Paise)</label>
            <input required type="number" name="price" value={form.price} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="5500000" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Seats Available</label>
            <input required type="number" name="seatsAvailable" value={form.seatsAvailable} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="40" />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-bold text-brand-dark">Baggage Allowance</label>
            <input required type="text" name="baggageAllowance" value={form.baggageAllowance} onChange={handleChange} className="w-full p-3 bg-[#f0f9ff]/50 border border-sky-100 rounded-xl" placeholder="e.g. 30KG Check-in + 7KG Cabin" />
          </div>
        </div>
